# Plugin CLI 101 Training Gate Aliases Plan

status: completed

## Context

The plugin had `npm run check`, `npm test`, and a root `make check` wrapper, but
it did not expose explicit lint or build aliases for the repository gate
contract.
