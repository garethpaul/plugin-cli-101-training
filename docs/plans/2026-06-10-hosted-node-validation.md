...
Add a commit-pinned, read-only hosted Linux and Windows matrix that runs `npm
test` without `npm install`. Preserve launcher modes, package files, example
immutability, unknown-key handling, and learner-name sanitization checks.
